...
A snapshot of where A11oy is heading. Issues are the source of truth — this is a navigational summary, not a contract.
...
- [ ] **UDS bundle reference deployment.** A complete, tested `uds-bundle.yaml` that boots A11oy plus a sample policy gate in a minimal UDS cluster.
...
- [ ] **Multi-party doctrine composition.** When two A11oy instances need to compose verdicts (e.g. coalition deployments), spell out and enforce the composition rules.
- [ ] **Hardware attestation hooks.** TPM / SEV-SNP attestation of the running A11oy binary tied back to the cosign signature.
...
- A11oy is **not** a model. It does not train, fine-tune, or host LLMs. It is a verdict layer on top of model outputs.
- A11oy is **not** an MLOps platform. It does not manage GPU schedulers, datasets, or experiments.
- A11oy does **not** ship telemetry to any vendor. The shipped payload makes zero outbound network calls. This is a doctrine, not a default.
