...
file. The token and its digest are never returned or logged; public identity
contains only an operator-selected ID.
"""
...
class BearerTokenAuthorizer:
    def __init__(self, *, token_id: str, token: bytes, source: str) -> None:
        if (
            ...
            or len(token_id.strip()) > 256
            or any(ord(character) < 0x20 for character in token_id)
        ):
