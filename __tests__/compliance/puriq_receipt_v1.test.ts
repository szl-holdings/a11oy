...
describe("PurIQ receipt payload v1", () => {
  ...
  test.each([
    "2026-09-04T23:59:60Z", "2026-06-29T23:59:60Z", "2026-12-30T23:59:60Z"
  ])("leap second on an impossible date %s is rejected by schema and client", (issuedAt) => {
    const receipt = clone(vector("single_receipt_session").receipts[0]);
    receipt.issued_at = issuedAt;
    expect(validate(receipt)).toBe(false);
    expect(receiptV1.validateReceipt(receipt)).toContain("issued_at");
  ...
  test("receipt validation rejects lone surrogates before hashing", async () => {
    const receipt = clone(vector("single_receipt_session").receipts[0]);
    receipt.subject.source_record_id = "\uD800";

    expect(receiptV1.validateReceipt(receipt)).toContain("unicode");
    await expect(receiptV1.computePayloadHash(receipt)).rejects.toThrow(/unicode/);
    const verdict = await receiptV1.verifySession([receipt]);
    expect(verdict.valid).toBe(false);
    expect(verdict.results[0].errors).toContain("unicode");
    expect(verdict.results[0].errors).not.toContain("payload_hash_uncomputable");
  });
