...
- Goal: make the FG-S1→S4 pipeline, DSSE-shaped receipts, and Putnam-to-FG wiring real without score inflation.
...
- `runtime/src/putnam_to_fg_wiring.ts` — maps the honest Putnam `score01` to FG-04 advisory-only evidence.
...
- `runtime/src/pipeline.test.ts` — tests for DSSE, Putnam wiring, pass/fail gate behavior, validation errors, hash linkage, deterministic receipts.
...
- Putnam remains the honest `1/12 = 8.3%` baseline and is advisory-only.
