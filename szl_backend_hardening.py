...
DEFAULT_FABRIC_NODES: List[Dict[str, Any]] = [
    ...
    {"name": "chaski", "kind": "tailnet-gpu", "sovereign": False,
     # chaski moved 100.76.58.50 -> 100.102.173.88 (confirmed live per AUTO_STATE).
     # Static fallback is the current tailnet IP so the box probes the right host even
     # when tailscale is unreadable; an explicit A11OY_GPU_NODE_CHASKI_IP env pin still
     # wins. reachable is set ONLY by a real TCP probe — never fakes green.
