...
3. Stale permissive RLS policies are removed before the one expected isolation
   policy is recreated on each application data table. The owner-only binding
   table is left RLS-enabled, NO FORCE, and policy-free.
...
8. User-settable tenant/domain GUCs no longer authorize a context alone. The
   ...
   under a temporary grant that has since been revoked. The preflight is
   independent of stale policies: it first converges ownership and restores
   ENABLE/NO FORCE RLS, so the owner scan observes physical rows. A rejection
   rolls those catalog changes back atomically.
9. The corrective migration itself restores the updated-at and append-only
   ...
   expected triggers, and restores the exact application-table and owner-only
   binding-table RLS states. These controls no longer depend on rerunning the
   historical migrations afterward.
...
- `python3 tests/test_memory_covenant_v2.py -v` — 71 tests passed.
- `python3 -m py_compile ...` — passed.
- PostgreSQL parser pass over all three migrations and acceptance SQL (base
  `68`, hardening `24`, corrective `74`, acceptance `43` statements) — passed.
- Embedded PostgreSQL `18.3` (`PGlite 0.5.5`) passed clean-install acceptance,
  reproduced a binding row hidden from its non-superuser owner by FORCE RLS and
  a GUC-filtered policy, rejected that row from the corrective migration,
  preserved the pre-existing owner/RLS/policy state atomically, proved temporary
  binding INSERT ACL and policy authority absent after revocation, rejected the
  planted row, passed the full second pass, and passed final acceptance — passed.
...
- Workflow YAML parse, `actionlint 1.7.12`, and `git diff --check` — passed.
