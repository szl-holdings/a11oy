...
# set on the a-11-oy.com deploy env (.env / docker-compose / systemd EnvironmentFile):
...
```bash
curl -s https://a-11-oy.com/api/a11oy/code/healthz | jq '.sovereign,.inference,.key_resolution.base_url'
...
DEADLINE: reply in forge-STATUS-20260612.md by Sat 2026-06-13 09:00 EDT. If no reply by then, the CTO agent will treat the box as not-flipped and keep a-11-oy.com honest as-is (sovereign:false until the box endpoint is live) — the demo runbook already plans for that.
