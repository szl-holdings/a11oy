...
## TWO PRE-EXISTING CI REDS for Forge to fix (NOT caused by this session — flagged honestly)
Both predate tonight's bundle work, are environment/toolchain-gated, and need the Forge build/cluster env — neither is a doctrine regression and neither blocks the published+signed bundle story:

1. **`Prove Bundle Install` + `Prove Organs` (szl-uds-deployment) — never passed in CI.** They ran for the first time today (triggered by the `uds-v0.3.0` tag) and failed for all 5 members at the "install from published bundle" / cluster step. Root cause is almost certainly the GitHub-hosted runner being unable to do the full k3d bring-up + cosign keyless verify (the Sigstore TUF root `tuf-repo-cdn.sigstore.dev` is egress-blocked in our sandboxes too — same 403). **Fix path = run them on the tower** (the offline-deploy proof above doubles as this proof). If you want them green in CI, the runner needs cluster capacity + Sigstore TUF egress (or `--insecure-ignore-tlog` is NOT acceptable — keep the real verify, run it where egress works).

2. **`Operational Validation` (a11oy) — chronic red since at least 2026-06-09, failing on EVERY run across many unrelated commits.** Last failure sha `1276ecac` ("extend live health check to cover console data tabs"). It is path-gated (`artifacts/a11oy-uds/**`, `deploy/**`, `scripts/*.py`, `huggingface/**`) so it does NOT run on most commits and is not a required merge check; the current a11oy HEAD has zero completed reds. The failing step "Validate operational surfaces" runs `validate-operational.sh` (npm receipt-substrate test + manifest/attestation verify) AND the `payload:bundle / payload:bundle:verify` chain — i.e. a node/pnpm build-artifact step that needs the full operational build env. **Forge: please reproduce `bash scripts/validate-operational.sh && npm run payload:bundle:verify` locally and fix whichever command exits non-zero** (likely a missing/*stale* build artifact or a receipt-substrate test). This is the one genuinely-red workflow in the estate worth clearing before the Day-3 outbrief so the CI board is all-green for judges who look.
