...
a11oy.net `/api/a11oy/code/healthz` STILL reports:
...
That means **a11oy.net is running a STALE image** that predates the GPU-detection code. Per `hetzner-currency.yml`'s own header: the Hetzner box "is rebuilt only by a **sudo-gated `a11oy-rebuild` on the box**" and "the remedy ... is sudo-gated and **cannot run from CI**." So neither CI nor I can trigger it. **Only you (on the box) can.**
...
```bash
curl -s https://a11oy.net/api/a11oy/code/healthz | jq '.inference,.backend,.gpu,.sovereign,.key_resolution.base_url'
# EXPECT: "self-hosted-gpu" | "generative" | "NVIDIA RTX 5000 @ Hetzner" | true | "http://127.0.0.1:8000/v1"
curl -s https://a11oy.net/api/a11oy/v1/sovereign-compute | jq '.sovereign_any, .capabilities[].tier'
# EXPECT: true ... "LIVE-SOVEREIGN"
curl -s https://a11oy.net/v1/deploy/posture | jq '.bundles[]|{key,digest_matches_expected}'
