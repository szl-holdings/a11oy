...
## VERIFIED STATUS (I checked live a-11-oy.com + GitHub + GHCR just now)
- **a-11-oy.com is UP and healthy** — `/api/a11oy/code/healthz` returns `status:ok, mode:live`. Good.
...
The app code is DONE and live — it auto-detects a local endpoint. You only need to (1) serve a model on the GPU and (2) set 2 env vars on the a-11-oy.com deployment. No code, no rebuild.
...
# 3) set TWO env vars on the a-11-oy.com service (the deployment that serves a-11-oy.com — k8s deploy env,
...
NOTE: the a-11-oy.com deployment env lives on the Hetzner host (the k8s `deploy/manifests/a11oy-deployment.yaml` env block, or the container's runtime env — wherever you set HF_TOKEN today). Set the two vars in that SAME place, restart, done.
...
I closed a gap: the orchestrator /api/a11oy/code/healthz now emits inference:"self-hosted-gpu", backend:"generative", sovereign:true, gpu:<label> WHENEVER A11OY_MODEL_BASE_URL is a non-router endpoint (commit 1af88fe0, byte-identical HF, doctrine-green). Verified honest today (still hf-router, sovereign:false). So the moment you set A11OY_MODEL_BASE_URL + A11OY_GPU_LABEL on the a-11-oy.com deploy and restart, BOTH the healthz AND the new szl_sovereign_compute pane (LIVE-MANAGED -> LIVE-SOVEREIGN) flip automatically — no further code needed. Just the 2 env vars + the vLLM container. The whole chain is staged.

## UPDATE (05:40 EDT) — a-11-oy.com is serving a STALE build; one redeploy fixes 3 things at once
I re-verified a-11-oy.com live just now. It is UP + healthy (`mode:live`), but it is running an OLDER image than `main`. Evidence:
...
- `/v1/deploy/posture` still shows the **mesh bundle `digest_matches_expected:false`** — the box has the stale `b2e4980f` expect-digest pin; I already re-pinned it to the live `50ebc519…` in commit `1bed19fa` (the **HF Space already shows `true`** — only a-11-oy.com is behind).
- The new **Sovereign Compute pane** (`/api/a11oy/v1/sovereign-compute`, wired in commits `ed42065b`+`b7a24ea5`+`6a15f9ae`) is live on the HF Space but absent on a-11-oy.com.

**So: when you redeploy a-11-oy.com from current `main` (HEAD `aee6ff88` or later) — the SAME deploy where you set the two GPU env vars — you get all of this in one shot:**
...
No code work for you — just `git pull` on the box / re-pull the image to `main`, set the 2 env vars, restart. Everything is already committed, CI-green, byte-identical HF, doctrine-clean. The HF Space is the reference for "what a-11-oy.com should look like after redeploy."
