...
def register(app, ns: str = "a11oy") -> dict:
    ...
    Additive. Prefers FastAPI's add_api_route; falls back to Starlette Route append.
    ...
    try:
        if callable(add_api_route):
            app.add_api_route(path, _h_eu_disclosure, methods=["GET"])
        else:
            app.router.routes.append(Route(path, _h_eu_disclosure))
