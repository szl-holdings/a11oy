...
The flagship UI is a thin governed presentation shell over already-shipped SZL live APIs.
It never fabricates data: upstream failures are rendered UNAVAILABLE. Existing A11oy,
...
def token_from_env() -> tuple[str, str]:
    for key in ("HF_ORG_TOKEN","HF_WRITE_TOKEN","HF_TOKEN","HUGGINGFACE_TOKEN","HUGGING_FACE_HUB_TOKEN"):
        value=os.getenv(key)
        if value and value.strip(): return value.strip(), key
...
@app.get("/api/live")
def live(): return read_upstream()
@app.get("/api/source")
def source(): return CFG
@app.get("/",response_class=HTMLResponse)
def root():
    title=CFG["title"]; vertical=CFG["vertical"]
    return f'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title} · SZL Holdings</title><style>
*{{box-sizing:border-box}}body{{margin:0;background:#030405;color:#f5f7fa;font:15px Inter,system-ui,sans-serif;min-height:100vh}}body:before{{content:"";position:fixed;inset:0;background:radial-gradient(circle at 75% 10%,rgba(90,180,255,.12),transparent 34%),linear-gradient(rgba(255,255,255,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.025) 1px,transparent 1px);background-size:auto,32px 32px,32px 32px;pointer-events:none}}main{{max-width:1180px;margin:auto;padding:42px 22px 70px;position:relative}}nav{{display:flex;justify-content:space-between;align-items:center;gap:15px;margin-bottom:70px}}.brand{{font-weight:800;letter-spacing:.14em}}.pill{{border:1px solid #2b3139;border-radius:999px;padding:8px 12px;color:#aab4c0;font:12px ui-monospace,monospace}}h1{{font-size:clamp(48px,8vw,104px);line-height:.92;margin:0 0 20px;letter-spacing:-.06em}}.k{{color:#7dd3fc;font:12px ui-monospace,monospace;letter-spacing:.17em}}.lede{{max-width:760px;color:#a9b2bc;font-size:19px;line-height:1.65}}.grid{{display:grid;grid-template-columns:1.3fr .7fr;gap:18px;margin-top:38px}}.card{{border:1px solid #252b32;background:rgba(10,12,15,.78);border-radius:18px;padding:22px;box-shadow:0 18px 60px rgba(0,0,0,.25)}}.status{{display:flex;align-items:center;gap:9px;font:12px ui-monospace,monospace}}.dot{{width:9px;height:9px;border-radius:50%;background:#64748b;box-shadow:0 0 18px currentColor}}.live .dot{{background:#7dd3fc}}pre{{white-space:pre-wrap;word-break:break-word;color:#cbd5e1;font:12px/1.6 ui-monospace,monospace;max-height:420px;overflow:auto}}a{{color:#e9f6ff}}button{{background:#f5f7fa;color:#050607;border:0;border-radius:10px;padding:11px 16px;font-weight:750;cursor:pointer}}.meta{{display:grid;gap:12px;color:#97a3af;font-size:13px}}@media(max-width:760px){{.grid{{grid-template-columns:1fr}}nav{{margin-bottom:44px}}}}</style></head><body><main><nav><div class="brand">SZL HOLDINGS</div><div class="pill">A11OY · EVIDENCE-BEARING VERTICAL</div></nav><div class="k">{vertical}</div><h1>{title}</h1><p class="lede">A flagship vertical surface backed by SZL's governed runtime. Live upstream evidence is rendered when reachable; missing evidence remains <b>UNAVAILABLE</b> rather than being fabricated.</p><div class="grid"><section class="card"><div id="st" class="status"><span class="dot"></span><span>PROBING</span></div><pre id="out">Loading governed live feed…</pre><button onclick="load()">Refresh evidence</button></section><aside class="card meta"><b>Canonical source</b><a href="{CFG['source']}" target="_blank">{CFG['source']}</a><b>Product portfolio</b><a href="{CFG['product']}" target="_blank">a-11-oy.com/products/</a><b>Evidence doctrine</b><span>MEASURED · REPORTED · MODELED · UNAVAILABLE are kept distinct.</span></aside></div></main><script>async function load(){{let st=document.getElementById('st'),out=document.getElementById('out');try{{let r=await fetch('/api/live',{{cache:'no-store'}}),j=await r.json();st.className='status '+(j.status==='LIVE'?'live':'');st.children[1].textContent=j.status+' · '+(j.latency_ms??'—')+' ms';out.textContent=JSON.stringify(j,null,2)}}catch(e){{st.children[1].textContent='UNAVAILABLE';out.textContent=String(e)}}}}load();setInterval(load,60000)</script></body></html>'''
'''
...
def main() -> int:
    token, token_source = token_from_env(); api=HfApi(token=token)
    receipt: dict[str,Any]={"schema":"szl.hf-vertical-flagships/v1","at":dt.datetime.now(dt.timezone.utc).isoformat(),"token_source":token_source,"spaces":[]}
    with tempfile.TemporaryDirectory() as td:
        root=Path(td)
        for cfg in FLAGSHIPS:
            rid=f'{ORG}/{cfg["slug"]}'; d=root/cfg["slug"]; d.mkdir()
            (d/"app.py").write_text(APP,encoding="utf-8"); (d/"Dockerfile").write_text(DOCKER,encoding="utf-8"); (d/"requirements.txt").write_text(REQ,encoding="utf-8"); (d/"config.json").write_text(json.dumps(cfg,indent=2)+"\n",encoding="utf-8"); (d/"README.md").write_text(readme(cfg),encoding="utf-8")
            item={"id":rid,"source":cfg["source"],"upstream":cfg["upstream"],"actions":[]}
            try:
                api.create_repo(repo_id=rid,repo_type="space",space_sdk="docker",exist_ok=True,private=False); item["actions"].append("ensure_public_docker_space")
                api.upload_folder(repo_id=rid,repo_type="space",folder_path=str(d),commit_message="feat: publish governed SZL vertical flagship"); item["actions"].append("publish_source")
                try: api.update_repo_settings(repo_id=rid,repo_type="space",private=False)
                except Exception: pass
                try: api.restart_space(rid); item["actions"].append("restart")
                except Exception as exc: item["restart_note"]=type(exc).__name__
                deadline=time.time()+360
                while time.time()<deadline:
                    try:
                        stage=str(api.get_space_runtime(rid).stage or "UNKNOWN").upper(); item["stage"]=stage
                        if stage=="RUNNING": break
                        if "ERROR" in stage: break
                    except Exception as exc: item["stage_error"]=type(exc).__name__
                    time.sleep(10)
