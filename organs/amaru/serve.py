...
DOCTRINE CONSTRAINTS & KEY INVARIANTS (Doctrine v11 LOCKED — do not drift)
-------------------------------------------------------------------------
* Canonical numbers are LOCKED: 749 declarations / 14 unique axioms (15 raw, 1 dup) /
  163 sorries (112 baseline + 51 Putnam) / 12 MCP tools / 46 policy gates. These appear in
  /api/amaru/v1/{honest,lambda,version,doctrine} and MUST NOT be bumped without a doctrine
  version change. Lutar lean kernel commit: c7c0ba17.
* Λ (lambda) is **Conjecture 1 — NOT a closed theorem**. There is an open CAUCHY_ND sorry
  (Lutar/Uniqueness.lean:120) plus a missing symmetry axiom. Every surface that mentions Λ
  uniqueness must say "Conjecture", never "Theorem/proven".
* /api/amaru/v1/honest is the deterministic honest-disclosure endpoint. Its payload semantics
  are a CONTRACT (declarations:749 etc.); do not alter its behavior in efficiency/comment passes.
* SLSA: self-claim is **L1 (honest)** in-Space; an L2 build-provenance attestation is roadmap via
  Wire D — NOT yet earned (.github/workflows/ghcr-build-push.yml wires it but it is not attached to
  the deployed image). L3 is NOT claimed (no hardened isolated build pipeline). szl_provenance
  reports SLSA_LEVEL="L1".
* DSSE receipt chain (szl_dsse + szl_provenance._KhipuDAG): each node digest =
  sha256(canonical_json(receipt) + parent_digest) so the chain is genuinely hash-linked and
  tamper-evident; envelopes are REAL ECDSA-P256-SHA256 cosign signatures when the
  SZL_COSIGN_PRIVATE_PEM runtime secret is present (else honestly UNSIGNED, never fabricated).
* Cardano-anchored receipts are demo-seeded, NOT on-chain mainnet (stated in /honest).
* Section 889 = exactly 5 vendors (Huawei, ZTE, Hytera, Hikvision, Dahua) — see the frontier
  /api/amaru/v1/doctrine payload; do not add/remove vendors.
* Per-file COPY discipline in the Dockerfile (NO `COPY . .`). Every module imported at startup
  must have a matching COPY line, or its try/except guard will silently degrade the surface.
"""
...
# /api/amaru/openapi.json, /healthz + /readyz (Khipu chain check), JSON logs
...
try:
    ...
    _be_report = _be_harden.harden(app, organ="amaru")
    import sys as _be_sys
    print(f"[amaru] BE hardening registered: {_be_report.get('registered')} "
          f"khipu={_be_report.get('khipu_backend')}", file=_be_sys.stderr)
except Exception as _be_e:
    import sys as _be_sys, traceback as _be_tb
    print(f"[amaru] BE hardening NOT registered: {_be_e!r}", file=_be_sys.stderr)
    _be_tb.print_exc()
...
try:
    ...
    # BUGFIX (fullstack pass #3): the f-string used doubled braces ({{...}}),
    # so it printed the literal text "{_prov_status}" instead of the value and
    # the boot diagnostic was useless. Single braces interpolate the real
    # status. Also corrected the SLSA label to the honest L1 (szl_provenance
    # reports SLSA_LEVEL="L1"; the chain is signed, L2 is roadmap via Wire D).
    # Log-only change; no route, contract, or /honest behavior touched.
    print(f"[amaru] szl_provenance registered (Wire D LIVE, SLSA L1 signed): {_prov_status}", file=sys.stderr)
...
async def amaru_honest() -> JSONResponse:
    ...
    # OWNS retrieval) + HONEST SLSA L1 (amaru image cosign-signed, public-verifiable:
    # Fulcio O=sigstore.dev, Rekor logIndex 1711945840). An SLSA L2 build-provenance attestation is roadmap (NOT yet earned)
    ...
    return JSONResponse({
        ...
        "slsa_evidence": {
            "level": "L1 honest (L2 roadmap via Wire D)", "image_tag": "uds-v0.2.0",
...
async def amaru_cortex_ask_killinchu() -> JSONResponse:
    ...
    return JSONResponse({
        ...
        "governance": "Doctrine v11 — Lambda is a Conjecture, signatures PLACEHOLDER, SLSA L1 honest (cosign-signed; L2 attestation roadmap, not yet claimed; not L3)",
    })
...
try:
    ...
    try:
        from amaru import retrieval as _amaru_retrieval
    except Exception:  # retrieval is additive; reason still works without it
...
async def amaru_v1_reason(request: Request) -> JSONResponse:
    ...
    # 1a) REAL RAG retrieval. If the caller supplied no citations, query a real
    # knowledge base (public arXiv Atom API) to discover grounding sources.
    # HONESTY OVER CHECKLIST: we never fabricate — if retrieval finds nothing,
    # user_citations stays empty and the guard below forces an honest refusal.
    retrieved: list = []
    if not user_citations and _amaru_retrieval is not None:
        try:
            rs = _amaru_retrieval.retrieve(question, k=3, timeout=12.0)
            retrieved = [r.to_dict() for r in rs]
            user_citations = [r["url"] for r in retrieved]
            # When the cortex self-retrieves, resolution MUST be enforced so we
            # only emit verdicts backed by a URL that actually returns 200.
            if user_citations:
                require_resolution = True
        except Exception:  # pragma: no cover - defensive
            retrieved = []
        steps.append({"step": "rag_retrieval", "engine": "arxiv-atom-api",
                      "sources": retrieved, "count": len(retrieved)})
    ...
    # Fold per-URL resolution back onto the retrieved sources so the UI can
    # show a truthful HTTP-200 badge per citation (never faked).
    if retrieved and isinstance(chk.resolved, dict):
        for _src in retrieved:
            _r = chk.resolved.get(_src.get("url")) or {}
            _src["resolved"] = _r.get("resolved")
            _src["status"] = _r.get("status")
    if not chk.ok:
...
# Premium cortex reasoning UI: GET /amaru/reason (chain-of-verification viz,
# Inca palette, mobile responsive). Registered on the ROOT app BEFORE the SPA
# catch-all so the exact path wins. ADDITIVE ONLY.
try:
    from amaru_reason_page import register_reason_page as _register_reason_page
    _register_reason_page(app)
    print("[amaru] /amaru/reason premium cortex UI registered", file=sys.stderr)
except Exception as _reason_page_err:  # pragma: no cover - defensive
    print(f"[amaru] /amaru/reason page NOT registered: {_reason_page_err}", file=sys.stderr)


# -------------------------------------------------------------------------
# PARITY ENDPOINTS (serve.py outer app)
# feat/parity-confidence-eval (Perplexity Computer Agent 2026-06-10)
# /api/amaru/v1/confidence  -- hallucination-risk + confidence scoring
# /api/amaru/v1/eval        -- RAG retrieval evaluation
# Parity: Fiddler AI, Arize Phoenix, LangSmith retrieval evals.
# amaru EDGE: Lambda-gated (13-axis geomean) + DSSE receipt on every score.
# ADDITIVE ONLY. Registered BEFORE app.mount so these paths win.
# Doctrine v11 LOCKED 749/14/163. Lambda = Conjecture 1 (NOT a theorem).
# Signed-off-by: stephenlutar2-hash <stephenlutar2@gmail.com>
# -------------------------------------------------------------------------
try:
    from amaru import confidence as _outer_confidence_mod
    from amaru import retrieval_eval as _outer_retrieval_eval_mod
    _OUTER_PARITY_OK = True
    _OUTER_PARITY_ERR = None
except Exception as _outer_parity_err:  # pragma: no cover - defensive
    _OUTER_PARITY_OK = False
    _OUTER_PARITY_ERR = repr(_outer_parity_err)
    import sys as _sys_parity
    print(f"[amaru] outer parity modules import failed: {_OUTER_PARITY_ERR}",
          file=_sys_parity.stderr)


@app.post("/api/amaru/v1/confidence")
async def outer_confidence_score(request: Request) -> JSONResponse:
    """Hallucination-risk + confidence scoring for a cortex output.

    Parity: Fiddler AI (faithfulness/hallucination), Arize Phoenix (evals),
    LangSmith (online evals). amaru EDGE: Lambda-gated + DSSE receipt.

    POST {question, answer, verification_answer?, axis_scores?}
    Returns confidence [0,1], sub-scores, hallucination_risk flag, receipt hash.
    """
    if not _OUTER_PARITY_OK:
        return JSONResponse(
            {"ok": False, "error": "confidence module unavailable",
             "detail": _OUTER_PARITY_ERR, "doctrine": "v11"}, status_code=503)
    try:
        body = await request.json()
    except Exception:
        body = {}
    ...
    if not question or not answer:
        return JSONResponse(
            {"ok": False, "error": "question and answer required", "doctrine": "v11"},
            status_code=400)
    result = _outer_confidence_mod.score(
        question, answer,
        verification_answer=body.get("verification_answer"),
        axis_scores=body.get("axis_scores"),
    )
    return JSONResponse(result.to_dict())


@app.post("/api/amaru/v1/eval")
async def outer_retrieval_eval(request: Request) -> JSONResponse:
    ...
    amaru EDGE: deterministic (auditable, no LLM API cost), receipt-hash-bound.

    POST {question, answer, chunks: [str, ...]}
    Returns four metrics [0,1], composite score, provenance receipt.
    """
    if not _OUTER_PARITY_OK:
        return JSONResponse(
            {"ok": False, "error": "retrieval_eval module unavailable",
             "detail": _OUTER_PARITY_ERR, "doctrine": "v11"}, status_code=503)
    try:
        body = await request.json()
    except Exception:
    ...
    if not question or not answer:
        return JSONResponse(
            {"ok": False, "error": "question and answer required", "doctrine": "v11"},
            status_code=400)
    result = _outer_retrieval_eval_mod.evaluate(question, answer, [str(c) for c in chunks])
    return JSONResponse(result.to_dict())
...
# === (white-screen iframe). Repointed to the LIVE a11oy 3D estate at a-11-oy.com/holographic        ===
...
<body>
<div class="topbar">
<div><strong>amaru . Khipu Constellation 3D</strong> <span style="color:#7f8db0">- live 3D estate (a-11-oy.com/holographic)</span> <span class="badge">source LIVE</span></div>
<div><a href="/">back</a> <a href="https://a-11-oy.com/holographic" target="_blank">open full view</a></div>
</div>
<iframe src="https://a-11-oy.com/holographic" allow="cross-origin-isolated; fullscreen" loading="lazy"></iframe>
</body></html>"""
...
async def serve_spa(path: str):
    # An unknown API-shaped path must return a structured JSON 404, NOT the SPA
    # HTML shell (which would make a client's JSON.parse throw "Unexpected token
    # '<'"). The original guard enumerated api/, v1/, v3/ but omitted v2/ — so an
    # unknown /v2/* path leaked index.html. Added v2/ for parity with the live
    # UNAY/Khipu-LMDB v2 surface. Behavior-preserving for every real route
    # (all real v2 endpoints live under /api/amaru/v2/* and are matched earlier).
    if (path.startswith("api/") or path.startswith("v1/")
            or path.startswith("v2/") or path.startswith("v3/")):
...
try:
    ...
    async def _ken_mcp_call(request: _SzlRequest):
        try:
            ...
            return _SzlJSON({"error": "invalid JSON body"}, status_code=400)
        if not isinstance(body, dict):
            return _SzlJSON({"error": "body must be a JSON object with a 'name' field"}, status_code=400)
...
try:
    ...
    _FTR_SLSA = "L1 honest (cosign-signed; verifiable via cosign verify). L2 build-provenance attestation is roadmap (Wire D) — not yet claimed. L3 not claimed."; _FTR_LAM = "Conjecture 1 (NOT a theorem; 163 sorries outstanding)"
...
# ============================================================================
# BEGIN: ELITE PARITY BLOCK — amaru (2026-06-04, Perplexity Computer Agent)
# Registers: F1–F23 formula registry, full LLM roster (a11oy parity),
# parity matrix, honest /healthz (SLSA L1 honest; L2 roadmap), 13-tab cortex console.
# ADDITIVE ONLY. Doctrine v11 LOCKED 749/14/163. Λ = Conjecture 1 (NOT theorem).
# Signed-off-by: stephenlutar2-hash <stephenlutar2@gmail.com>
# Co-Authored-By: Perplexity Computer Agent <agent@perplexity.ai>
# ============================================================================
try:
    import amaru_parity_elite as _parity_elite
    _parity_elite_status = _parity_elite.register(app)
    import sys as _pe_sys
    print(f"[amaru-elite] parity endpoints registered: {_parity_elite_status.get('registered', [])}", file=_pe_sys.stderr)
except Exception as _pe_e:
    import sys as _pe_sys, traceback as _pe_tb
    print(f"[amaru-elite] parity_elite NOT registered: {_pe_e!r}", file=_pe_sys.stderr)
    _pe_tb.print_exc(file=_pe_sys.stderr)

try:
    import amaru_cortex_console as _console_mod
    _console_mod.register_console(app)
    import sys as _cc_sys
    print("[amaru-elite] cortex console registered at GET /cortex-console", file=_cc_sys.stderr)
except Exception as _cc_e:
    import sys as _cc_sys, traceback as _cc_tb
    print(f"[amaru-elite] cortex_console NOT registered: {_cc_e!r}", file=_cc_sys.stderr)
    _cc_tb.print_exc(file=_cc_sys.stderr)
# ============================================================================
# END: ELITE PARITY BLOCK — amaru
# ============================================================================
