...
def register(app, organ: str, web_dir: str | None = None) -> dict[str, Any]:
    ...
    unified = here / "operator-unified.html"  # ADDITIVE: unified 4-pane shell (Yachay/Perplexity Computer Agent)
    ...
    @app.get(f"{p}/inbox")
    async def _inbox():
        items = list(_INBOX.get(organ, []))
        return JSONResponse({"inbox": items, "total": len(items), "doctrine": "v11"})  # P3 fix: wrapped shape
    ...
    async def _receipts(since: str | None = None, limit: int = 50):
        ...
        return JSONResponse({"receipts": rows, "total": len(rows), "note": "PLACEHOLDER signing disclosure: receipt signatures are SHA-256 digests pending real ECDSA-P256 key injection", "doctrine": "v11"})  # P3 fix: wrapped shape
    ...
    # --- ADDITIVE: Unified 4-pane Operator Shell at /unified (2026-06-01) ----
    # New route, registered here (before any SPA catch-all, same as /operator).
    # Serves web/operator-unified.html. Does NOT touch the existing /operator.
    async def _serve_unified():
        if unified.exists():
            return FileResponse(str(unified))
        return JSONResponse({"error": "operator-unified.html not deployed"}, status_code=404)
    app.get(f"/{organ}/unified")(_serve_unified)
    app.get("/unified")(_serve_unified)
    # --- end /unified ---
