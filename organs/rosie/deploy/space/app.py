...
# continuity) + DSSE/Cosign-signed Khipu receipts (SLSA L1 honest + L2 attested (Wire D LIVE; SLSA Provenance v1, cosign keyless-verified)).
...
async def _rosie_honest():
    ...
    # quorum) + HONEST SLSA L2 (rosie image verified public-verifiable: Fulcio
...
# rosie_jarvis.register() self-lifts its routes to the FRONT of _rosie_api.router so
# they win over `_rosie_api.mount("/api/rosie", ...)` (line ~2437) and the catch-all
# `_rosie_api.mount("/", ...)` (line ~2645) — Starlette resolves in order, front wins.
