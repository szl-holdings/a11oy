...
class PhaseBPayloadTests(unittest.TestCase):
    def test_exact_current_main_observation_paths_are_closed(self) -> None:
        ...
        self.assertEqual(KEVGATE_PATHS, frozenset({"/api/a11oy/v1/sec/kev"}))

    def test_every_phase_b_surface_gets_the_supplied_observation_clock(self) -> None:
        for path in PHASE_B_OBSERVATION_PATHS | PHASE_B_OBSERVATION_ALIASES:
            with self.subTest(path=path):
                source = {"ok": True, "nested": {"fetched_at": "older-source-time"}}
                ...
                self.assertEqual(result["nested"]["fetched_at"], "older-source-time")
    ...
    def test_bundled_kev_snapshot_is_canonical_cached_with_separate_detail(self) -> None:
        source = {
            "data_kind": "sample",
            "note": "Verbatim real KEV entries from the bundled catalog snapshot.",
            "count": 28,
        }
        result = normalize_phase_b_payload(
    ...
    def test_kev_error_response_is_not_rewritten_as_cached_evidence(self) -> None:
        source = {"error": "upstream unavailable", "data_kind": "sample"}
        result = normalize_phase_b_payload(
            "/api/a11oy/v1/sec/kev",
            source,
            status_code=503,
...
class PhaseBMiddlewareTests(unittest.TestCase):
    ...
    def test_middleware_adds_real_clock_and_preserves_nonvalidator_headers(self) -> None:
        app = _FakeApp()
        install_phase_b_response_contract(app)
    ...
    def test_middleware_canonicalizes_kev_without_false_live_label(self) -> None:
        app = _FakeApp()
        install_phase_b_response_contract(app)
        ...
        request = SimpleNamespace(url=SimpleNamespace(path="/api/a11oy/v1/sec/kev"))
        ...
        self.assertEqual(payload["detail"], response_payload_note := "Bundled snapshot; no live catalog fetch occurred.")
        self.assertEqual(payload["note"], response_payload_note)
        self.assertNotIn("observed_at", payload)

    def test_non_json_response_is_returned_without_consuming_its_body(self) -> None:
        app = _FakeApp()
        install_phase_b_response_contract(app)
        response = _FakeResponse(
            {"ok": True},
        ...
        request = SimpleNamespace(url=SimpleNamespace(path="/api/a11oy/provenance"))
