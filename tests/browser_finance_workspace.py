        ...
        with sync_playwright() as p:
            ...
            for width in (320, 375, 768, 1440):
                ...
                def route_handler(route):
                    ...
                    if path not in ("/api/source","/api/finance/providers","/api/live") and not path.startswith("/api/finance/observations/"):
                        unexpected.append(path); route.abort(); return
                    requests.append({"path":path,"query":parts.query,"method":req.method})
                    check(req.method == "GET", "non-read browser request")
