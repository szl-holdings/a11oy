...
from scripts.alert_channel_canary import EndpointContractError, evaluate, infer_mode, validate_endpoint
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        type(self).received.append({"path": self.path, "headers": {key.lower(): value for key, value in self.headers.items()}, "body": body})
...
def test_presence_only_cannot_claim_delivery_health() -> None:
    with _server() as (url, _):
        result = evaluate(url=url, should_send=False, requested_mode="ntfy", allow_insecure_loopback=True)
...
def test_slack_request_is_json() -> None:
    with _server() as (url, handler):
        result = evaluate(url=url, should_send=True, requested_mode="slack", allow_insecure_loopback=True)
...
def test_ntfy_request_is_text_with_ntfy_headers() -> None:
    with _server() as (url, handler):
        result = evaluate(url=url, should_send=True, requested_mode="ntfy", allow_insecure_loopback=True)
        received = handler.received[0]
    assert result.state == "HEALTHY"
...
def test_timeout_is_terminal_and_one_attempt() -> None:
    with _server(delay=0.3) as (url, _):
        result = evaluate(url=url, should_send=True, requested_mode="generic-json", timeout_seconds=0.05, allow_insecure_loopback=True)
...
def test_response_size_is_bounded() -> None:
    with _server(status=200, response=b"x" * 200) as (url, _):
        result = evaluate(url=url, should_send=True, requested_mode="generic-json", max_response_bytes=64, allow_insecure_loopback=True)
