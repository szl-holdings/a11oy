...
SCRIPT = ROOT / ".github/scripts/validate_frontier_source_pin_candidate_v2.py"
SPEC = importlib.util.spec_from_file_location(
    "validate_frontier_source_pin_candidate_v2",
    SCRIPT,
)
...
AUTHORITY_WORKFLOW = ROOT / ".github/workflows/frontier-source-pin-authority-v2.yml"
PROTECTION_DOC = ROOT / ".github/BRANCH_PROTECTION.md"
FIXTURE_ROOT = ROOT / "tests/fixtures/frontier_source_pin_v2"
PRE_REPAIR_FIXTURES = {
    validator.SOLO_WORKFLOW: FIXTURE_ROOT / "pre_frontier-solo-qualification.yml",
    validator.BUILDER_WORKFLOW: FIXTURE_ROOT
    / "pre_frontier-v16-7-exact-source-builder.yml",
    validator.CONTRACT: FIXTURE_ROOT / "pre_SOLO_EXECUTION_CONTRACT.json",
    validator.V1_TEST: FIXTURE_ROOT
    / "pre_test_validate_frontier_source_pin_candidate.py",
}
TARGET_V1_TEST_FIXTURE = (
    FIXTURE_ROOT / "target_test_validate_frontier_source_pin_candidate.py"
)
...
def _copy_tree(destination: Path) -> None:
    for relative in COPY_PATHS:
        ...
        shutil.copyfile(PRE_REPAIR_FIXTURES.get(relative, ROOT / relative), target)
...
"""
    ...
    shutil.copyfile(TARGET_V1_TEST_FIXTURE, candidate / validator.V1_TEST)
...
def test_transition_fixtures_are_exact_and_immutable() -> None:
    expected = {
        PRE_REPAIR_FIXTURES[validator.SOLO_WORKFLOW]:
            "706fba1353d402d7bc6ae3c20485b6e5ad77e46003e96992aceaad9996ac0294",
        PRE_REPAIR_FIXTURES[validator.BUILDER_WORKFLOW]:
            "3aa55e852c343c3a34560d306836f3c2d7da3042063483772701049f02325d63",
        PRE_REPAIR_FIXTURES[validator.CONTRACT]:
            "bb8c1a3f10ab92219df04f991d32f651db31f27650a0a4d602016c65b88a21ae",
        PRE_REPAIR_FIXTURES[validator.V1_TEST]:
            "f80c6578ea6da6402204ef6d65ac85fe09dbae97da5cf2d432b07315911a92b6",
        TARGET_V1_TEST_FIXTURE:
            validator.APPROVED_CANDIDATE_SHA256[validator.V1_TEST],
    }
    observed = {path: validator._sha256(path.read_bytes()) for path in expected}
    assert observed == expected


def test_three_file_transition_without_fixture_fix_fails_closed(
    tmp_path: Path,
) -> None:
    protected = tmp_path / "protected"
    candidate = tmp_path / "candidate"
    ...
    _copy_tree(candidate)
    _approved_candidate(candidate)
    (candidate / validator.V1_TEST).write_bytes(
        (protected / validator.V1_TEST).read_bytes()
    )
    try:
        ...
        raise AssertionError("three-file transition was admitted without fixture repair")
...
def test_authority_workflow_executes_only_protected_stdlib_code() -> None:
    ...
    protected_ref_condition = (
        "job.workflow_ref == "
        "'szl-holdings/a11oy/.github/workflows/frontier-source-pin-authority-v2.yml@refs/heads/main'"
    )
    ...
    assert (
        ...
        '".github/workflows/frontier-source-pin-authority-v2.yml"'
        in source
    )
    ...
    assert (
        "python -I -B .github/scripts/validate_frontier_source_pin_candidate_v2.py"
        in source
    )
    assert "--changed-paths-file frontier-source-pin-v2.changed-paths" in source
    assert '--workflow-ref "$WORKFLOW_REF"' in source
    assert '--workflow-sha "$WORKFLOW_SHA"' in source
    assert (
        "if test -f protected-validator/frontier-source-pin-authority-v2.json" in source
    )
