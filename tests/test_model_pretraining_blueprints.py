...
class BlueprintProjectionTests(unittest.TestCase):
    ...
    def test_untrained_format_hints_never_project_as_admitted_artifacts(self):
        for marker in ("model-blueprint", "not-trained"):
            for format_tags in (["peft", "safetensors"], ["gguf", "llama.cpp"],
                                ["transformers", "safetensors"]):
                with self.subTest(marker=marker, format_tags=format_tags):
                    raw = view.canonical(fixture_manifest([marker, *format_tags]))
                    result = view.project(raw, [], now=NOW)
                    model = result["models"][0]
                    self.assertEqual(model["categoryHint"], RECIPE)
                    self.assertIs(model["weightsVerified"], False)
                    self.assertIs(model["runtimeVerified"], False)
                    self.assertIs(model["trainingAllowed"], False)
