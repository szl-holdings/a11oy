# Dockerfile rationale notes

Full comment blocks condensed out of `Dockerfile` on 2026-07-16 to keep it under
Hugging Face's hard 128 KiB Dockerfile limit (builds die pre-log with exit 255 above it;
empirically isolated and reproduced on probe Spaces). Each §N anchor below is referenced
from a one-line marker at the original location.

## §1 — Install Node 22 (for a11oy serve TypeScript runner)
...
## §4 — Build cache-bust 2026-06-08T23:30Z (Wave23 instillation): knowledge.json was
...
## §5 — Ayllu — a11oy-native agent community. a11oy_ayllu.py is imported (guarded) by
...
## §6 — Wave 18 numerical-engine frontier. These are only the Apache-2.0 host contract
...
## §7 — M1 experimental model operational gate. Metadata and the status UI are
...
## §8 — routers/ — Wave-K Dev4 serve.py decomposition (first bounded slice). serve.py
...
## §10 — Copy serve orchestrator and gates manifest
...
## §11 — ADDITIVE (Dev0, 2026-06-14): SHARED szl3d 3D toolkit + holographic shell. The
...
## §12 — ADDITIVE (Wave-P Dev2): live cross-node mesh orchestrator backing the /holographic
...
## §13 — ADDITIVE (cathedral unification, GitHub-aligned): the ONE canonical genius
...
## §14 — ADDITIVE (holographic front-door landing, Dev1): the governed-inference-field
...
## §15 — ADDITIVE: batch-2 sovereign security data module (imported by serve.py; try/except-guarded
...
## §16 — ADDITIVE: a11oy Code IDE page (served by orchestrator GET /api/a11oy/code/ide as a
...
## §17 — ADDITIVE (Yachay / AYNI-OS, 2026-06-01): reciprocity organism + event-sourced replay
...
## §18 — ADDITIVE (Live 3D Wires / PURIQ Doctrine v12, Yachay): explicit per-file COPY.
...
## §19 — ADDITIVE (Provenance Hardening / Wire D + DSSE Cosign REAL signing, 2026-06-01, Yachay):
...
## §20 — ADDITIVE (UNAY + Khipu-LMDB v2, 2026-06-01, Yachay / Perplexity Computer Agent):
...
## §21 — ADDITIVE (Anatomy 3D + live formula wiring, 2026-06-02, Yachay / Perplexity
...
## §22 — ADDITIVE (Graph/Viz lane + Perplexity Computer Agent, 2026-06-06): AIR-GAP
...
## §23 — ADDITIVE (V4 Fleet Panel + /api/health fix, 2026-06-02, Dev2 Inti):
...
## §24 — ADDITIVE (Frontier wave, 2026-06-08): two founder tabs served from /app/web/
...
## §26 — ADDITIVE (nonlinear-PINN frontier, 2026-07-02): szl_pinn_nonlinear.py is imported by
...
## §27 — ADDITIVE (Lane F1, 2026-06-14): the 3D/holographic SUBSTRATE demo page, served at
...
## §28 — ADDITIVE (Lane A AGENTIC CORE, Dev A, 2026-06-14; QA9 restore 2026-06): the
...
## §29 — Wave M / Dev 4: Sovereign Local Model panel — imported GUARDED by serve.py

```
# Wave M / Dev 4: Sovereign Local Model panel — imported GUARDED by serve.py
# (GET /api/a11oy/v1/frontier/sovereign) AND by the /healthz sovereign rollup. Per the
# transitive COPY-guard (Wave L), this module MUST be COPY'd or the endpoint 404s and
...
## §30 — Wave P / Dev 4: three new frontier synthesis surfaces — imported GUARDED by serve.py
...
## §31 — Wave P / Dev 4: three more frontier surfaces — imported GUARDED by serve.py (GET
...
## §32 — Wave O / Dev 3: Brain-Body panel — imported GUARDED by serve.py (GET /api/a11oy/v1/
...
## §33 — FlowBrain frontier surface backend (WaveS) — imported by serve.py (guarded) for GET
...
## §34 — DEV2 Build 1: TEE/TDX attestation hook (2026-06-30) — imported by serve.py (guarded);
...
## §35 — BRAIN NERVOUS-SYSTEM HUB (WAVE O Dev1, 2026-07-07) — the pulse bus. serve.py imports
...
## §36 — ADDITIVE (Cross-Harness Receipt Bridge — Hermes + OpenClaw; 2026-06-01, Yachay /
...
## §37 — ADDITIVE (SZL Ken Agent Pattern v1, CTO Yachay Convergence Cycle 1, 2026-06-03):
...
## §38 — ADDITIVE (Formulas → Ecosystem instillation, Opus 4.8, 2026-06-03):
...
## §39 — FIX (formula/* 404 repair): a11oy_formula_endpoints.py imports a11oy.formulas.{allodial,
...
## §40 — ADDITIVE (Formulas SECTION page — closeout): serve.py imports a11oy_formulas_page
...
## §41 — ADDITIVE (Missing modules fix, 2026-06-04, Perplexity Computer Agent):
...
## §42 — ADDITIVE (Parity Gaps + Receipt Substrate fix, 2026-06-05, Orchestrator Squad):
...
## §43 — Formula-wiring module (ADDITIVE 2026-06-06): registers the kernel-verified theorem
...
## §44 — a11oy Code engine (governed chat/code/research; C20/W7-5 router; W5-3/W7-4 conformal;
...
## §45 — a11oy.code 7-tier organ->model router (TIERS + route() + tiers_payload()).
...
## §46 — a11oy Seismic forecaster (Doctrine v13): honest Reasenberg-Jones (1994) +
...
## §47 — Warhacker mission tabs backend (5 investor-facing surfaces; reuses
...
## §48 — Warhacker EXHAUSTIVE demos backend (5 full step-by-step demos: step timeline,
...
## §50 — LIVE CPU demo tier: install llama.cpp + fetch ONE tiny Apache-2.0 GGUF
...
## §51 — The wheel is BIND-MOUNTED from the builder stage (not COPY'd) so it is
...
## §52 — GGUF weight — RELIABLY PRESENT (pinned revision + retry + integrity verify), NOT best-effo
...
## §53 — RESILIENCE: on a constrained builder (A11OY_REQUIRE_LOCAL_LLM!=1, e.g. HF cpu-basic)
...
## §54 — ── BUILD-TIME EXTERNAL-DOWNLOAD AUDIT ───────────────────────────────────────
...
## §55 — ADDITIVE (Live-Data Layer, 2026-06-06, Warhacker): SHARED live-feed proxy module
...
## §56 — ADDITIVE (Investor-WOW Layer, 2026-06-08, Dev1): a11oy_dev1_endpoints.py exposes
...
## §57 — ADDITIVE (Vertical Packs Layer, 2026-06-08, Dev2): a11oy_vertical_feeds.py exposes
...
## §58 — ADDITIVE (Deep Feeds Layer, 2026-06-08, Dev-A): a11oy_deva_feeds.py exposes the 10
...
## §59 — ADDITIVE (Provenance & Trust Anchor, 2026-06-08): a11oy_amaru_feeds.py exposes the
...
## §60 — ADDITIVE (MINED UPGRADES, 2026-06, Yachay): four self-contained operator surfaces,
...
## §61 — ADDITIVE (RE-SWEEP WAVE 2, 2026-06, Yachay): four MORE operator surfaces from the
...
## §62 — ADDITIVE (WAVE9/10 INSTILLATION, 2026-06): the "Proven Formulas (experimental)"
...
## §65 — ADDITIVE (Task: HF Dataset Bucket Foundation): the ONE shared Hugging Face
...
## §67 — --- GOVERNANCE / EVAL / CALIBRATION layer (Dev B, 2026-06): ADDITIVE ---
...
## §68 — GOVERNED AUTO-REVIEW (Integration I2) — keystone autonomy layer: governed +
...
## §69 — GRC ALIGNMENT surface (Lane I5) — in-product ISO 42001 / NIST AI RMF / 800-53 /
...
## §70 — HOLOGRAPHIC 3D ENERGY SHOWCASE (Lane energy/06, 2026-06-14). The shared szl3d 3D
...
## §71 — feat/a11oy-models: live external model intel (LMArena Elo + HF Hub + Pareto) for llm/arena
...
## §72 — Standalone a11oy holographic energy page (/energy-holographic) + the upgraded HF energy
...
## §73 — Grid Energy Harvest honest dashboard (/energy-harvest); served via _ptg_serve from /app/we
...
## §74 — a11oy /code GOVERNED RUN-LOOP view (2026-07-06): standalone sovereign page (0 CDN)
...
## §75 — GOVERNED MODEL-HARNESS (Wave F module + Wave G /code wire-in, 2026-07-07):
...
## §76 — GOVERNED AGENT LOOP (Wave J, Dev 5): szl_agent_loop_governed.py COMPOSES the /code
...
## §77 — GOVERNED RAG · retrieval-with-receipts (Wave J · Dev 4, 2026-07-07):
...
## §78 — SOVEREIGN FLYWHEEL BRIDGE (Wave M · Dev 2, 2026-07-07): szl_sovereign_flywheel.py is
...
## §79 — Wave O (Dev 4): the BRAIN vault as a first-class RAG corpus source. Imported by
...
## §80 — WAVE-Q FRONTIER INDEX (honest ecosystem catalog + self-audit). Per-file COPY (this
...
## §81 — WAVE-S DEV 5: WHAT'S NEW (honest auto-derived estate changelog). Per-file COPY (this
...
## §82 — HONESTY WALL (feat/frontier-honestywall) — per-file COPY (this Dockerfile has NO
...
## §83 — BRAIN MEMORY FRESHNESS (feat/frontier-brainmemory) — per-file COPY (this Dockerfile has NO
...
## §84 — AGENT OS MAP (feat/frontier-agentos) — per-file COPY (this Dockerfile has NO
...
## §85 — BRAINGROUND (feat/frontier-brainground) — per-file COPY (this Dockerfile has NO
...
## §86 — BRAIN CONSENSUS (feat/frontier-brainconsensus) — per-file COPY (this Dockerfile has NO
...
## §87 — BRAIN QUERY AUDIT (feat/frontier-brainqueryaudit) — append-only, hash-linked ledger
...
## §88 — BRAIN LINEAGE (feat/frontier-brainlineage) — NODE-ORIGIN lineage over the SAME
...
## §89 — BRAIN EXPLAIN (feat/frontier-brainexplain) — per-file COPY (this Dockerfile has NO
...
## §90 — BRAIN GAPS (feat/frontier-braingaps) — per-file COPY (this Dockerfile has NO
...
## §91 — WAVE R Dev 1 — boot-resilience env/secret preflight. Per-file COPY (this
...
## §92 — FORGE-FAMILY WALL (2026-07-14): /api/forge/family — server-side ed25519
...
## §93 — KHIPU DEMO TAB (2026-07-16): /api/khipu/demo — three RECORDED Khipu navigator
...
## §94 — KHIPU DEMO PAGE (2026-07-16): a11oy_khipu_demo_nav.py attaches the idempotent
...
## §95 — QUANT SIGNALS WALL (2026-07-16): /api/quant/signals + /signals — the quant
...
## §96 — git_sha wireup (FORGE-INSTRUCTION-gitsha-quiet-window): surface the deployed commit
...
## §97 — The Second Brain's SQLite index is rebuildable, but a mounted /app/data keeps
...
## §98 — Build cache-bust 2026-06-05T00:00Z (Orchestrator Squad):
...
## §99 — Build cache-bust 2026-06-06T09:00Z (model-integration squad, Opus 4.8):
