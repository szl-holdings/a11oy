...
| Corpus card + digest gate | PutnamBench, miniF2F, MATH/AIMO-style corpora | Respect dataset licenses; store pointers/digests where redistribution is not allowed. | Immutable benchmark corpus cards with source URI, license, canonicalization, digest, count, and sealed state. | `pnpm benchmark:audit` |
| Raw-point Putnam ledger | Public contest scoring norms | No “cracked Putnam” claim without sealed corpus and judges. | `earned / possible` raw-score ledger with failures, retries, tool use, and judge disagreement. | `pnpm benchmark:audit` |
