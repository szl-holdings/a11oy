# Forking A11oy into your own UDS catalog

This document is for organizations — Defense Unicorns being the obvious one — who want to fork A11oy into their own GitHub organization, re-sign it with their own cosign key, and republish it under their own UDS catalog.
...
You consume the upstream `uds-v0.1.x` tarball as-is. Your `uds-bundle.yaml` references the upstream signed package. Your end users verify against the upstream `a11oy-uds-dev.pub` (or, in `v0.2.0`, against Sigstore Fulcio).
...
```bash
# 1. Fork on GitHub UI: https://github.com/szl-holdings/a11oy → Fork → <your-org>/a11oy
git clone https://github.com/<your-org>/a11oy
cd a11oy
...
git remote add upstream https://github.com/szl-holdings/a11oy
git fetch upstream
...
# 3. Verify locally
sha256sum dist/a11oy-uds/a11oy-uds-*.tar.zst
cosign verify-blob \
  --key /path/to/your-org-cosign.pub \
  --signature dist/a11oy-uds/a11oy-uds-*.tar.zst.sig \
  dist/a11oy-uds/a11oy-uds-*.tar.zst

# 4. Stage the verifier assets next to the tarball
cp /path/to/your-org-cosign.pub dist/a11oy-uds/your-org-a11oy-uds.pub
...
PUBLIC_BASE=https://github.com/<your-org>/a11oy/releases/download/uds-vX.Y.Z \
...
| File | Change |
|---|---|
| `README.md` | Add a "this is a re-signed fork of `szl-holdings/a11oy`" header with link to upstream |
| `.github/CODEOWNERS` | Replace SZL maintainers with your org's |
| `artifacts/a11oy-uds/zarf.yaml` | `metadata.name` should include your org prefix (e.g. `org-a11oy-uds-multi`) |
| `artifacts/a11oy-uds/scripts/build.sh` | Update the cosign key path comment if you keep keys elsewhere |
| `dist/a11oy-uds/*.pub` | Replace with your pubkey |
| `OPERATOR-QUICKSTART.md` (regenerated by build) | Update the BASE URL and pubkey filename references |

## What you should NOT change

- The doctrine code under `packages/a11oy-core/` — these are the invariants we collectively rely on. If you find a doctrine bug, file it upstream as a [doctrine question](https://github.com/szl-holdings/a11oy/issues/new?template=doctrine_question.yml) and we will respond on the science.
...
- **Trademark.** "A11oy" is a trademark of SZL Holdings, LLC. Your fork should use a distinct distribution name (e.g. `unicorn-a11oy`, `org-a11oy`) so end users can tell them apart.
...
```bash
BASE=https://github.com/<your-org>/a11oy/releases/download/uds-vX.Y.Z
curl -fsSLO $BASE/<your-org>-a11oy-uds-X.Y.Z.tar.zst
curl -fsSLO $BASE/<your-org>-a11oy-uds-X.Y.Z.tar.zst.sha256
curl -fsSLO $BASE/<your-org>-a11oy-uds-X.Y.Z.tar.zst.sig
curl -fsSLO $BASE/<your-org>-a11oy-uds.pub
sha256sum -c <your-org>-a11oy-uds-X.Y.Z.tar.zst.sha256
cosign verify-blob \
  --key <your-org>-a11oy-uds.pub \
  --signature <your-org>-a11oy-uds-X.Y.Z.tar.zst.sig \
  <your-org>-a11oy-uds-X.Y.Z.tar.zst
zarf package inspect definition <your-org>-a11oy-uds-X.Y.Z.tar.zst
```
