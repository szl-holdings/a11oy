 ...
 *
 * Doctrine v11 — sorry_undisclosed is a validation ERROR, not a warning.
...
function validatePayload(payload: Record<string, unknown>): string[] {
  ...
  // Doctrine v11: sorry_undisclosed is an explicit violation
  if (payload.lean_build_status === "sorry_undisclosed") {
    errors.push(
      "lean_build_status 'sorry_undisclosed' violates Doctrine v11 — all sorries must be named with discharge routes"
    );
  }
