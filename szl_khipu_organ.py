...
def status() -> Dict[str, Any]:
    payload: Dict[str, Any] = {
        ...
        "proof": {"url": _PROOF},
        "holdings": {"url": _HOLDINGS, "kind": "js-silhouette"},
...
def register(app, ns: str = "a11oy") -> Dict[str, Any]:
    ...
    try:
        ...
        for p in prefixes:
            app.router.routes.insert(0, Route(f"{p}/healthz", _json(healthz), methods=["GET"]))
            app.router.routes.insert(0, Route(f"{p}/status", _json(status), methods=["GET"]))
            app.router.routes.insert(0, Route(f"{p}/lambda", _lambda, methods=["POST"]))
            app.router.routes.insert(0, Route(f"{p}/greenlight", _green, methods=["POST"]))
            app.router.routes.insert(0, Route(f"{p}/anatomy", _anatomy, methods=["POST"]))
            app.router.routes.insert(0, Route(f"{p}/prefix", _prefix, methods=["POST"]))
            app.router.routes.insert(0, Route(f"{p}/route", _route, methods=["POST"]))
            routes.extend(
                [
                    f"{p}/healthz",
                    f"{p}/status",
