...
# Readiness-harness artifacts. The contract matrix (tabs.json) and the optional
# probe verdict are generated by tools/readiness-harness/ and may or may not be
# bundled with a given deploy — loaders fail soft and the endpoint reports honestly.
_HARNESS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "tools", "readiness-harness")
_TAB_MATRIX_PATHS = (
    os.environ.get("SZL_TAB_MATRIX_PATH", ""),
    os.path.join(_HARNESS_DIR, "tabs.json"),
)
_PROBE_VERDICT_PATHS = (
    os.environ.get("SZL_PROBE_VERDICT_PATH", ""),
    os.path.join(_HARNESS_DIR, "readiness-verdict.json"),
)


def _load_json_file(paths) -> Optional[Dict[str, Any]]:
    for p in paths:
        if not p:
            continue
        try:
...
def _load_tab_matrix() -> Optional[Dict[str, Any]]:
    return _load_json_file(_TAB_MATRIX_PATHS)


def _load_probe_verdict() -> Optional[Dict[str, Any]]:
    v = _load_json_file(_PROBE_VERDICT_PATHS)
    if v is None:
        return None
    v = dict(v)
    v["available"] = True
...
def register(app, ns: str = "a11oy") -> None:
    ...
    @app.get(base + "/tab-matrix")
    async def _readiness_tab_matrix():  # noqa: ANN202
        """Serve the readiness-harness contract matrix (tabs.json) plus, when a
        probe verdict has been run to disk, the latest "Lies?" summary. Honest:
        if neither artifact is present we say so rather than fabricate a pass."""
        matrix = _load_tab_matrix()
        verdict = _load_probe_verdict()
        if matrix is None:
            return JSONResponse({
                "layer": "%s readiness tab-matrix" % ns,
                "honest": _HONEST,
                "available": False,
