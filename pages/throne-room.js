...
// SZL canonical mobile helpers (inline — additive; OrbitControls is touch-native).
const SZL_MOBILE = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0);
const SZL_REDUCED = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
function szlSetVH(){ document.documentElement.style.setProperty('--vh', (window.innerHeight*0.01)+'px'); }
szlSetVH(); window.addEventListener('resize', szlSetVH); window.addEventListener('orientationchange', szlSetVH);
...
function makeWebGL2(){
  RENDER_MODE = 'webgl2';
  return new THREE.WebGLRenderer({ canvas: freshCanvas(), antialias:!SZL_MOBILE, alpha:false, powerPreference: SZL_MOBILE?'low-power':'high-performance' });
}
...
function animate(){
  ...
  HEROES.forEach(h=>{
    ...
    if(!SZL_REDUCED){ m.core.rotation.y += 0.004; m.ring.rotation.z += 0.006; }
