...
checks enforce the doctrine-v11 honesty fix:
  ...
  * a fresh process (empty chain) -> honest UNAVAILABLE, no fabricated artifact;
  * after a REAL composite is POSTed, the tile READS that latest digest but remains
    UNAVAILABLE until a cryptographic DSSE signature verifies in this process;
...
def test_concept_tile_reads_chain_and_reverifies():
    ...
    # Integrity-only Khipu records and DSSE_PLACEHOLDER metadata are never
    # promoted to signed evidence.
    assert concept["label"] == fm.UNAVAILABLE
    assert concept.get("signature_verified") is False


def test_empty_chain_is_honest_unavailable_no_artifact():
    dag = _prov_dag()
    if dag.depth() != 0:
...
def test_real_post_grows_chain_then_get_reads_it():
    ...
    concept = next(t for t in m["capabilities"] if t["category"] == "frontier-concept")
    # A real POST proves that a hash-chained artifact exists. It does not prove
    # authorship: the current DSSE_PLACEHOLDER is not a signature.
    assert concept["label"] == fm.UNAVAILABLE
    assert concept.get("signature_verified") is False
...
if __name__ == "__main__":
    ...
    test_empty_chain_is_honest_unavailable_no_artifact()
